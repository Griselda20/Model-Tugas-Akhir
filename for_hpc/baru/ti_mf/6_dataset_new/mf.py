...
common_model_kwargs = dict(
    cnn_drop_path_rate = stochastic_depth_rate,
    dw_conv = 'dw',
    kernel_size=(3, 3),
    cnn_exp = (6, 4),
    cls_token_num = 1,
...
id_dict = {}
for i, line in enumerate(open('/home/tasi2425111/resized-tiny-imagenet-200/wnids.txt', 'r')):
...
train_transform = transforms.Compose([
    RandAugment(num_ops=randaugment_n, magnitude=randaugment_m, interpolation=transforms.InterpolationMode.NEAREST),
    transforms.ToTensor(),
...
train_dataset = TrainTinyImageNetDataset(id=id_dict, transform = train_transform)
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=2, pin_memory=True)

val_dataset = ValTinyImageNetDataset(id=id_dict, transform=val_transform)
...
patience = 10
counter = 0
save_path = "best_model_mobile_former.pth"
...
def train(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs=train_epochs):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            counter = 0
            checkpoint = {
                'epoch': epoch + 1,                          # Menyimpan epoch terakhir
